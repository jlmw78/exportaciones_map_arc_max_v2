...
function drawCharts() {
  // Continentes
  $(function() {
    var result = alasql("SELECT continente AS label, SUM(kilos) AS total FROM ? GROUP BY continente", [features]);
  ...
  // Paises
  $(function() {
    var result = alasql("SELECT pais AS label, SUM(kilos) AS total FROM ? GROUP BY pais ORDER BY total DESC", [features]);
    var chart = c3.generate({
...
// Basemap Layers
var mapboxOSM = L.tileLayer("http://tile.openstreetmap.org/{z}/{x}/{y}.png", {
  maxZoom: 19,
  //~ subdomains: ["a", "b", "c", "d"],
  attribution: 'Basemap <a href="https://www.openstreetmap.org/copyright" target="_blank">© OpenStreetMap contributors, CC-BY-SA</a>'
});

var mapboxSat = L.tileLayer("https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}", {
  maxZoom: 19,
  //~ subdomains: ["a", "b", "c", "d"],
  attribution: 'Basemap <a href="https://www.google.at/permissions/geoguidelines/attr-guide.html" target="_blank">Map data ©2015 Google</a>'
});
